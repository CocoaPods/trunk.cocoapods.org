      ...
      describe 'with URLs' do
        ...
        it 'pod_created calls call correctly' do
          Webhook.expects(:write_child).once.with(
            ...
            %Q(+0100","data_url":"some_url"};pod_created_url1,pod_created_url2\n)
          )
          Webhook.pod_created(Time.parse('01-01-2001'), 'some_url')
        end
        it 'version_created calls call correctly' do
          Webhook.expects(:write_child).once.with(
            %Q(version;create;{"type":"version","action":"create","timestamp":"2001-01-01 00:00:00 ) +
            %Q(+0100","data_url":"some_url"};version_created_url1,version_created_url2\n)
          )
          Webhook.version_created(Time.parse('01-01-2001'), 'some_url')
        end
        it 'spec_updated calls call correctly' do
          Webhook.expects(:write_child).once.with(
            %Q(spec;update;{"type":"spec","action":"update","timestamp":"2001-01-01 00:00:00 ) +
            %Q(+0100","data_url":"some_url"};spec_updated_url1,spec_updated_url2\n)
          )
          Webhook.spec_updated(Time.parse('01-01-2001'), 'some_url')
        end
      end
