...
EOYAML
    ...
    it "does not allow updates to submission job's if the client isn't authorized" do
      post '/travis_build_results', { 'payload' => fixture_read('TravisCI/pull-request_success_payload.json') }, { 'HTTP_AUTHORIZATION' => 'incorrect token' }
    ...
    it "does not break with a normal commit Travis build status notification" do
      post '/travis_build_results', { 'payload' => fixture_read('TravisCI/commit_payload.json') }, { 'HTTP_AUTHORIZATION' => Travis.webhook_authorization_token }
      last_response.status.should == 200
      @job.reload.travis_build_success?.should == nil
      @job.should.not.needs_to_perform_work
    end
    ...
    it "does not break with a Travis build status notification for an unknown pull-request" do
      post '/travis_build_results', { 'payload' => fixture_read('TravisCI/pull-request_unknown_payload.json') }, { 'HTTP_AUTHORIZATION' => Travis.webhook_authorization_token }
      last_response.status.should == 200
      @job.reload.travis_build_success?.should == nil
      @job.should.not.needs_to_perform_work
    end
    ...
    it "updates the submission job's Travis build status as passing the lint process" do
      post '/travis_build_results', { 'payload' => fixture_read('TravisCI/pull-request_success_payload.json') }, { 'HTTP_AUTHORIZATION' => Travis.webhook_authorization_token }
    ...
    it "updates the submission job's Travis build status as failing the lint process" do
      post '/travis_build_results', { 'payload' => fixture_read('TravisCI/pull-request_failure_payload.json') }, { 'HTTP_AUTHORIZATION' => Travis.webhook_authorization_token }
      last_response.status.should == 204
